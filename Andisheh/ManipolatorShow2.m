function ManipolatorShow2(DH)
n = size(DH,1);
m1=max(DH(:,2));
m2=max(DH(:,3));
mm1=min(DH(:,2));
mm2=min(DH(:,3));
am=(abs(m1-mm1)+abs(m2-mm2))/2;
nm=0;
if am>1
    while(am>1)
        am=am/10;
...
T=eye;
    for i = 1:n
        ...
        if i==n
            d =[d(:,:),(D-2*10^nm)*(z(:,i)-o(:,i))+o(:,i)];
            a =[a(:,:),A*(x(:,i+1)-o(:,i+1))+d(:,i+1)];
            g =[10^nm*(z(:,i+1)-o(:,i+1))+a(:,i+1),10^nm*(y(:,i+1)-o(:,i+1))+(10^nm*(z(:,i+1)-o(:,i+1))+a(:,i+1)),10^nm*(z(:,i+1)-o(:,i+1))+(10^nm*(y(:,i+1)-o(:,i+1))+(10^nm*(z(:,i+1)-o(:,i+1))+a(:,i+1))),-10^nm*(y(:,i+1)-o(:,i+1))+(10^nm*(z(:,i+1)-o(:,i+1))+a(:,i+1)),10^nm*(z(:,i+1)-o(:,i+1))+(-10^nm*(y(:,i+1)-o(:,i+1))+(10^nm*(z(:,i+1)-o(:,i+1))+a(:,i+1)))];
            xp = {'k','--',2};
            yp = {'k',':',2};
            zp = {'k','-',2};
