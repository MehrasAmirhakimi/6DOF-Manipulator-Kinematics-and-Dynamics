function [sol, realindex] = PieperIK5DOF(DH, T)
...
if DH(5, 1) ~= -pi/2 || DH(5, 2) ~= 0 || DH(5, 3) ~= 0
    alphat = 0;
    if DH(5, 1) ~= -pi/2
        alphat = DH(5, 1) + pi/2;
    end
...
for i = 1:4
   ir = isreal(sol(i, 1)) + isreal(sol(i, 2)) + isreal(sol(i, 3));
    if ir == 3
        ...
        % Modified for alpha4 = +/-pi/2, alpha5 = +/-pi/2
        theta4 = atan2(R(1, 2)/(S(alpha4)*S(alpha5)), -R(2, 2)/(S(alpha4)*S(alpha5)));
        theta5 = atan2(R(3, 1)/S(alpha4), -R(3, 3)/(S(alpha4)*S(alpha5)));
