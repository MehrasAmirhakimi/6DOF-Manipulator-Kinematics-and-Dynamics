function [sol, realindex] = PieperIK5DOF2(DH, T)
...
for i = 1:4
   ir = isreal(sol(i, 1)) + isreal(sol(i, 2)) + isreal(sol(i, 3));
    if ir == 3
        ...
        theta4 = atan2(R(1, 3)/(S(alpha4)*C(alpha5)), -R(2, 3)/(S(alpha4)*C(alpha5)));
        theta5 = atan2(R(3, 1)/S(alpha4), R(3, 2)/(S(alpha4)*C(alpha5)));
